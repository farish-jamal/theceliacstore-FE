            ...
            />
              ...
              {hasItems && (() => {
                const waMessage = encodeURIComponent(
                  ...
                  items.map((item: any) => `${item.quantity ?? 1}x ${item.product?.name ?? ''} - \u20B9${item.price}`).join('\n') +
                  `\n\nTotal: \u20B9${finalPrice}\nPlease confirm availability and delivery.`
                );
                const waLink = `https://wa.me/919810107887?text=${waMessage}`;
                return (
                  <a
                    href={waLink}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full flex items-center justify-center gap-2 border border-[#25D366] text-[#25D366] text-sm py-2.5 rounded-lg mt-2"
                  >
